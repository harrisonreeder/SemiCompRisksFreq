...
#3. a function to output complete prediction based on (1) and (2)
#4. a function for illness-death models to (optionally) compute allllll the stuff
#h1 h2 h3, H1 H2 H3, S1 S2 S3, cond and marg
...
get_jac <- function(tseq, xnew, beta, eta,
  ...
  # browser()
  stopifnot(func_type %in% c("logh","h","logH","H"))
  stopifnot(tseq[1]>0)
...
predict.Freq_HReg2 <- function (object, xnew = NULL,
  ...
  if (object$class[2] == "Surv") {
    ...
    #first, generate all of the "raw materials" (h,H,S, and standard errors)
...
#I should just rewrite this using matplot
