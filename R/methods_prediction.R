...
#'                            para, nP0, nP, frailty,
#'                            model, h3_tv="none",h3tv_knots=NULL,
#'                            hazard, knots_list, n_quad, quad_method,
#'                            Finv, alpha){
#'   # browser()
#'   marg_flag <- is.null(frailnew)
...
#'                 S2=NULL,se_S2=NULL, ll_S2=NULL, ul_S2=NULL,
#'                 S3=NULL,se_S3=NULL, ll_S3=NULL, ul_S3=NULL,
#'                 h1_marg=NULL, h2_marg=NULL,
#'                 h3_marg=NULL, h3_marg0=NULL, h3_marg_curr=NULL,
#'                 # H1_marg=NULL, H2_marg=NULL, H3_marg=NULL,
#'                 # S1_marg=NULL, S2_marg=NULL, S3_marg=NULL,
#'                 p_term_only=NULL, se_p_term_only=NULL,
#'                 p_nonterm=NULL, se_p_nonterm=NULL,
#'                 p_nonterm_only=NULL, se_p_nonterm_only=NULL,
#'                 p_both=NULL, se_p_both=NULL,
...
#'   #next, compute conditional probabilities
#'   #note, for the moment these are length(tseq) by length(eta) matrices!
#'   value$p_neither <- exp(-value$H1 - value$H2)
#'
#'   S1_b <- exp(-value$H1)
#'   S1_a <- rbind(1, S1_b[-length(tseq),,drop=FALSE])
#'   S2_b <- exp(-value$H2)
#'   S2_a <- rbind(1, S2_b[-length(tseq),,drop=FALSE])
#'   value$p_term_only <- 0.5 *
#'     apply(S1_a*S2_a + S1_b*S2_a - S1_a*S2_b - S1_b*S2_b, MARGIN = 2, cumsum)
#'   value$p_nonterm <- 0.5 *
#'     apply(S2_a*S1_a + S2_b*S1_a - S2_a*S1_b - S2_b*S1_b, MARGIN = 2, cumsum)
#'
...
#'
#'     H1_b <- value$H1
#'     H1_a <- rbind(0, H1_b[-length(tseq),,drop=FALSE])
#'     H2_b <- value$H2
#'     H2_a <- rbind(0, H2_b[-length(tseq),,drop=FALSE])
#'
#'     #Here, we interchange the integral of gamma with the summation of trapezoid rule
#'     #to get sum of "integrated integrands"
#'     value$p_term_only_marg <- 0.5 * apply(
#'       (1 + theta * (H1_a + H2_a))^(-1/theta) +
#'         (1 + theta * (H1_b + H2_a))^(-1/theta) -
#'         (1 + theta * (H1_a + H2_b))^(-1/theta) -
#'         (1 + theta * (H1_b + H2_b))^(-1/theta), MARGIN=2, cumsum)
#'     value$p_nonterm_marg <- 0.5 * apply(
#'       (1 + theta * (H1_a + H2_a))^(-1/theta) +
#'         (1 + theta * (H1_a + H2_b))^(-1/theta) -
#'         (1 + theta * (H1_b + H2_a))^(-1/theta) -
#'         (1 + theta * (H1_b + H2_b))^(-1/theta), MARGIN=2, cumsum)
...
#'
#'     #eventually, these could be "re-predicted" to ensure that timing and t1 are correctly
#'     #accounted for, but for now we'll just assume that timing is equally spaced...
#'     #and further assume that t1 is not included as a covariate...
#'     if(tolower(model) %in% c("m","markov")){
#'       #last row of H3[1:i] is H3(t2i), so
#'       #compute matrix of H3(t2) - H3(t1) for every t1 between 0 and t1i
...
#'       #take the first i entries, and then reverse their order
#'       #to get a vector that increments from H3(t2i-0) to H3(t2i-t2i)
#'       H3_a_sub <- value$H3[1:i,,drop=FALSE][i:1,,drop=FALSE]
#'     }
#'     H3_b_sub <- rbind(H3_a_sub[-1,,drop=FALSE],0)
#'
#'     S1_a_sub <- S1_a[1:i,,drop=FALSE]
#'     S1_b_sub <- S1_b[1:i,,drop=FALSE]
#'     integrand_a_sub <- S2_a[1:i,,drop=FALSE] * exp(-H3_a_sub)
#'     integrand_b_sub <- S2_b[1:i,,drop=FALSE] * exp(-H3_b_sub)
#'     value$p_nonterm_only[i,] <- 0.5 *
...
#'       H2_a_sub <- H2_a[1:i,,drop=FALSE]
#'       H2_b_sub <- H2_b[1:i,,drop=FALSE]
#'       value$p_nonterm_only_marg[i,] <- 0.5 * colSums(
#'         (1 + theta * (H1_a_sub + H2_a_sub + H3_a_sub))^(-1/theta) +
#'           (1 + theta * (H1_a_sub + H2_b_sub + H3_b_sub))^(-1/theta) -
#'           (1 + theta * (H1_b_sub + H2_a_sub + H3_a_sub))^(-1/theta) -
#'           (1 + theta * (H1_b_sub + H2_b_sub + H3_b_sub))^(-1/theta))
...
#'     value$h3_marg0 <- value$h3 * (1+theta) / (1+theta*value$H3)
#'
#'     #now, create a "long" format two-dimensional time structure, for
#'     #mapping function over (t1,t2) grid.
#'     index_mat <- expand.grid(1:NROW(value$h3),
#'                              1:NROW(value$h1))
#'
#'     #Marginal hazard of h3 also depends on t1, so we calculate it for every
#'     #combination.
#'     if(tolower(model) %in% c("m","markov")){
#'       #in markov model, only care about region where t2 >= t1
#'       index_mat <- index_mat[index_mat[,1]>=index_mat[,2],]
#'       value$tseq_h3_marg <- rbind(
#'         cbind(time1=0,time2=tseq),
#'         cbind(time1=tseq[index_mat[,2]],time2=tseq[index_mat[,1]]))
#'       value$h3_marg <- rbind(
#'         value$h3_marg0,
#'         value$h3[index_mat[,1],,drop=FALSE] * (1+theta) /
#'           (1 + theta * (value$H1[index_mat[,2],,drop=FALSE] +
#'                           value$H2[index_mat[,2],,drop=FALSE] +
#'                           value$H3[index_mat[,1],,drop=FALSE] -
#'                           value$H3[index_mat[,2],,drop=FALSE])))
#'       #finally, add in a table showing the "instantaneous" h3 for every point
#'       #as though t1 has "just" occurred.
#'       value$h3_marg_curr <- value$h3_marg[value$tseq_h3_marg[,"time1"]==value$tseq_h3_marg[,"time2"],,drop=FALSE]
#'
#'     } else{
#'       #for semi-markov model, time grid is from every point t1 to every point t1+t2
#'       value$tseq_h3_marg <- rbind(
#'         cbind(time1=0,time2=tseq),
#'         cbind(time1=tseq[index_mat[,2]],
#'               time2=tseq[index_mat[,2]] + tseq[index_mat[,1]]))
#'
#'       value$h3_marg <- rbind(
#'         value$h3_marg0,
#'         value$h3[index_mat[,1],,drop=FALSE] * (1+theta) /
...
#'                           value$H3[index_mat[,1],,drop=FALSE])))
#'
#'       #unlike with markov model, the "instantaneous" hazard doesn't really
#'       #make sense for semi-markov models.
#'
#'     }
#'   }
#'
...
#'
#' #' @export
#' pred_helper_ID2 <- function(tseq, x1new=NULL, x2new=NULL, x3new=NULL, frailnew=NULL,
#'                            para, nP0, nP, frailty, model,
#'                            hazard, knots_list, n_quad, quad_method,
#'                            Finv, alpha){
#'   # browser()
...
#'   #make it so that you've "started integrating" from time tseq[1]
#'   #in practice, this is no different if tseq = 0
#'   p_neither0 <- exp(-value$H1[1,] - value$H2[1,])
#'   value$p_neither <- t(t(value$p_neither) / p_neither0)
#'
#'   #instead of assuming that there's an implicit 0, just directly use the first thing!
#'   #this allows for left truncation by just beginning the integral at an arbitrary time
#'   S1_b <- exp(-value$H1[-1,,drop=FALSE])
#'   S1_a <- exp(-value$H1[-length(tseq),,drop=FALSE])
#'   S2_b <- exp(-value$H2[-1,,drop=FALSE])
#'   S2_a <- exp(-value$H2[-length(tseq),,drop=FALSE])
#'
#'   #and then in turn, we know that wherever the integral is starting from, it's
#'   #gotta start at 0, so just add that back
#'   value$p_term_only <- rbind(0, 0.5 *
...
#'     #make it so that you've "started integrating" from time tseq[1]
#'     #in practice, this is no different if tseq = 0
#'     p_neither0_marg <- exp( (-1/theta) * log1p(theta * (value$H1[1,] + value$H2[1,])) )
#'     value$p_neither_marg <- t(t(value$p_neither_marg) / p_neither0_marg)
#'
#'
#'     # H1_b <- value$H1
...
#'     # H2_b <- value$H2
#'     # H2_a <- rbind(0, H2_b[-length(tseq),,drop=FALSE])
#'     H1_b <- value$H1[-1,,drop=FALSE]
#'     H1_a <- value$H1[-length(tseq),,drop=FALSE]
#'     H2_b <- value$H2[-1,,drop=FALSE]
#'     H2_a <- value$H2[-length(tseq),,drop=FALSE]
#'
#'     #Here, we interchange the integral of gamma with the summation of trapezoid rule
#'     #to get sum of "integrated integrands"
#'     value$p_term_only_marg <- rbind(0, 0.5 * apply(
...
#'
#'       #YAY THIS UPDATE WORKS!! EVEN FOR LEFT TRUNCATED and IRREGULARLY SPACE DATA!
#'       temp <- t( -t(value$H3[1:(i+1),,drop=FALSE]) + value$H3[(i+1),])
#'       H3_b_sub <- temp[-1,,drop=FALSE]
#'       H3_a_sub <- temp[-(i+1),,drop=FALSE]
#'
#'     } else{
...
#'       #reprediction solves the above issue
#'       #only lingering issue is how to bring in dependence on t1
#'       H3_temp <- pred_helper_uni(tseq = tseq[(i+1)] - tseq[1:(i+1)],
#'                                     xnew = x3new,
#'                                     phi = para[nP0_start[3]:nP0_end[3]],
#'                                     beta = para[nP_start[3]:nP_end[3]],
#'                                     hazard = hazard, knots_vec = knots_list[[3]],
...
#now, write a sort of 'specialized' function that gives prediction results as an array,
#following the format that I used to use.
